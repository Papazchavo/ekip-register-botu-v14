...
module.exports = {
  ...
  run: async (client, message, args) => {  
    ...
    if (!args[0]) return message.reply({ embeds: [new EmbedBuilder()
      ...
      .setDescription(`> ${red} Bir argüman belirtmelisin (ekle/kaldır/liste)!`)
      ] }).then((e) => setTimeout(() => { e.delete(); }, 5000));  

    if (args[0] === "oluştur" || args[0] === "ekle") {
        let komutAd = args[1];
        message.react(red)
        if (!komutAd) return message.reply({ embeds: [new EmbedBuilder()
          .setThumbnail()
          .setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) })
          .setDescription(`${red} Bir komut adı belirlemelisin.`)
          ] }).then((e) => setTimeout(() => { e.delete(); }, 5000));  
          message.react(red)
        if (!args[2] && !args[3]) return message.reply({ embeds: [new EmbedBuilder()
          .setThumbnail()
          .setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) })
          .setDescription(`> ${red} Bir yetkili ve verilecek rolü belirlemelisin. \` Yetkili Rol - Verilecek Rol \``)
          ] }).then((e) => setTimeout(() => { e.delete(); }, 5000));  
        ...
        let kanalcık = args[4];
        if (!kanalcık) return message.reply({ embeds: [new EmbedBuilder()
          .setThumbnail()
          .setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) })
          .setDescription(`> ${red} Bir kanal ismi belirlemelisin. Örnek: role_log kanal isim olarak girin`)
          ] }).then((e) => setTimeout(() => { e.delete(); }, 5000));  

        let talents = await özelPerms.findOne({ guildID: message.guild.id, komutAd: komutAd });
        message.react(red)
        if (talents) return  message.reply({ embeds: [new EmbedBuilder()
          .setThumbnail()
          .setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) })
          .setDescription(`> ${red} Bu isimde bir komut zaten mevcut`)
          ] }).then((e) => setTimeout(() => { e.delete(); }, 5000));  
        ...
        message.react(red)
        if (!data2) return message.reply({ embeds: [new EmbedBuilder()
          .setThumbnail()
          .setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) })
          .setDescription(`> ${red} ${red} Hangi komutu silmek istiyorsun`)
          ] }).then((e) => setTimeout(() => { e.delete(); }, 5000));  
